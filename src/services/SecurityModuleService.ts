...
export class HsmWalletService {
  ...
  private startSession(readOnly: boolean = true): Buffer {
    ...
    const mode = readOnly ? pkcs11.CKF_SERIAL_SESSION : pkcs11.CKF_RW_SESSION | pkcs11.CKF_SERIAL_SESSION;
    ...
    if (this.config.HSM_CONFIG.pin) { // CKF_LOGIN_REQUIRED
        this.cryptoki.C_Login(session, pkcs11.CKU_USER, this.config.HSM_CONFIG.pin);
        this.loggedIn = true;
    }
    return session;
  }
  ...
  public async generate(): Promise<string> {
    ...
    try {
      ...
      const publicKeyTemplate = [
        ...
        { type: pkcs11.CKA_EC_PARAMS, value: new Buffer("06052b8104000A", "hex") }, // secp256k1 params 06052b8104000A
      ];
      ...
      const mechanism: pkcs11.Mechanism = { mechanism: pkcs11.CKM_EC_KEY_PAIR_GEN, parameter:  new Buffer("06052b8104000A", "hex")};
      const keys = this.cryptoki.C_GenerateKeyPair(session, mechanism, publicKeyTemplate, privateKeyTemplate);
      const ecPointDerEnc = this.cryptoki.C_GetAttributeValue(session,
                                                              keys.publicKey,
                                                              [{type: pkcs11.CKA_EC_POINT}])[0].value! as Buffer;
      console.log(ecPointDerEnc.length);
      console.log(ecPointDerEnc.toString("hex"));
      await this.addressSchema.create({"privateKeyId": privateKeyLabel,
  ...
  public async sign(ecPoint: string, hash256: string): Promise<[string, string]> {
    const session = this.startSession();
    try {
      const res: any = await this.addressSchema.findOne({ where: {"ecPoint": ecPoint}});
      if (res === null) {
        throw "HSM does not have key for " + ecPoint;
      }
      const addressModel: AddressModel = res.dataValues;

      this.cryptoki.C_FindObjectsInit(session, [{ type: pkcs11.CKA_LABEL, value: addressModel.privateKeyId}]);
      ...
      const privateLabel = this.cryptoki.C_GetAttributeValue(session,
                                                             privateKey,
                                                             [{type: pkcs11.CKA_LABEL}])[0].value! as Buffer;

      console.log("private key label: ",  privateLabel.toLocaleString());

      this.cryptoki.C_FindObjectsInit(session, [{ type: pkcs11.CKA_LABEL, value: addressModel.publicKeyId}]);
      ...
      const publicLabel = this.cryptoki.C_GetAttributeValue(session,
                                                            publicKey,
                                                            [{type: pkcs11.CKA_LABEL}])[0].value! as Buffer;
      ...
      this.cryptoki.C_SignInit(session, { mechanism: pkcs11.CKM_ECDSA, parameter: null }, privateKey);
      const signature = this.cryptoki.C_Sign(session, msgBuffer256, new Buffer(1024));

      this.cryptoki.C_VerifyInit(session, { mechanism: pkcs11.CKM_ECDSA, parameter: null }, publicKey);
